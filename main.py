...
def calc_bending_angle(base_joint, middle_joint, tip_joint):
    ...
    norm_product = np.linalg.norm(
        vector_base_to_middle) * np.linalg.norm(vector_middle_to_tip)
    angle_radians = np.arccos(
        dot_product / norm_product) if np.arccos(dot_product/norm_product) is not None else 0
    ...
    if results.multi_hand_landmarks is not None:
        ...
        for hand_index, hand in enumerate(results.multi_handedness):
            ...
            if hand_label_matches[mp_hands.HandLandmark.WRIST] != HAND_LABEL:
                cv.putText(
                    frame, "Use la mano derecha", (20, 20), cv.FONT_HERSHEY_SIMPLEX, 0.6, mp_drawing.BLUE_COLOR, 2)
                continue
            ...
            # correct position
            if hand_landmarks[0].y < hand_landmarks[1].y or hand_landmarks[0].y < hand_landmarks[5].y or hand_landmarks[0].y < hand_landmarks[9].y or hand_landmarks[0].y < hand_landmarks[13].y or hand_landmarks[0].y < hand_landmarks[17].y:
                cv.putText(
                    frame, "Coloque la mano en una posicion correcta", (20, 20), cv.FONT_HERSHEY_SIMPLEX, 0.6, mp_drawing.BLUE_COLOR, 2)
                return
            ...
            for idx, points in enumerate(hand_landmarks):
                ...
                if idx in HAND_LANDMARKS:
                    cv.circle(frame, (x_real, y_real),
                              2, mp_drawing.RED_COLOR, 2)

            # fingers coordinates
            thumb_coords = np.array([keypoints[2], keypoints[3], keypoints[4]])
            index_coords = np.array([keypoints[5], keypoints[6], keypoints[8]])
            middle_coords = np.array(
                [keypoints[9], keypoints[10], keypoints[12]])
            ring_coords = np.array(
                [keypoints[13], keypoints[14], keypoints[16]])
            pinky_coords = np.array(
                [keypoints[17], keypoints[18], keypoints[20]])
            ...
            cv.putText(frame, f"Suavizado: {angle_smoothed}",
                       (20, 120), cv.FONT_HERSHEY_SIMPLEX, 0.6, mp_drawing.BLUE_COLOR, 2)
            cv.putText(frame, f"Medio: {middle_angle}",
                       (200, 120), cv.FONT_HERSHEY_SIMPLEX, 0.6, mp_drawing.RED_COLOR, 2)
